...
def fire_bullet(ai_settings, screen, ship, bullets):
    ...
    #Ограничение на создание пуль, проверяем условие
...
def update_bullets(ai_settings, screen, stats, sb, ship, aliens, bullets):
    bullets.update()
    #удаляем пули, вышедшие за край экрана
...
def get_number_aliens_x(ai_settings, alien_width):
    """Вычисляет количество инопланетян в ряду"""
    available_space_x = ai_settings.screen_width - 2*alien_width
    number_aliens_x = int(available_space_x/(2*alien_width))

    return number_aliens_x

def creare_alien(ai_settings, screen, aliens, alien_number, row_number):
...
def create_fleet(ai_settings, screen, ship, aliens):
    ...
    for row_number in range(number_rows):
        for alien_number in range(number_aliens_x):
            creare_alien(ai_settings, screen, aliens, alien_number, row_number)
...
def update_aliens(ai_settings, screen, stats, ship, aliens, bullets):
    #Проверка края, после обновление позиции всех пришельцев
    check_fleet_edges(ai_settings,aliens)
    #Обновляет позиции всех пришельцев во флоте
    aliens.update()
    #Проверка коллизий - при столкнвении корабль уничтожается
    if pygame.sprite.spritecollideany(ship, aliens):
        ship_hit(ai_settings, screen, stats, ship, aliens, bullets)
        # print("О Черт, спасите наши задницы!!") :)))))))))))
...
def ship_hit(ai_settings, screen, stats, ship, aliens, bullets):
    #обрабатываем столкновение корабля и пришельца
...
def check_aliens_bottom(ai_settings,screen, stats, ship, aliens,bullets):
    #проверяем, добрались ли пришельцы до нижнего края
